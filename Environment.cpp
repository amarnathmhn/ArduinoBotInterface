...
Point Environment :: getGridSlotCenter( int x, int y){
	...
	if(!withGrid){
		printf("Environment::getGridSlotCenter: do 'setGrid(enable, slotSize)' to enable grid operations\n");
		return Point(-1,-1);
	}
...
void Environment :: drawGrid(bool showCenter){
	...
	if( !withGrid ){
		printf("Environment::drawGrid(): do setGrid(true,<slotSize>) to perform grid operations\n");
	}
	...
	int line_size = 3;
 	al_draw_rectangle(x, y, x + columns*slotSize , y + rows*slotSize, al_map_rgb(0,255,0) , 5);
	...
	for(int i = 1; i <= rows - 1; i++){
	 	al_draw_line(x,y + i*slotSize, x + columns*slotSize, y + i*slotSize,
			     al_map_rgb(0,255,0),line_size);
	}
